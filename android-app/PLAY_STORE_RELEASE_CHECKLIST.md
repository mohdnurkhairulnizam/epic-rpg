...
The Android project is packaged as a Capacitor application with package identity `com.epicrpg.familyquest`, version name `1.0`, version code `1`, minimum SDK 24, and target/compile SDK 36. The generated debug APK is suitable for private sideload testing. The generated release AAB is **unsigned** because no owner keystore or Play credentials were available in the workspace; it is not ready for direct Play Console upload until the owner completes signing.
...
| Data safety | Complete the Data safety form based on the final build and the owner’s policy choices. Local storage and any future integrations must be represented accurately; do not claim that the app collects or shares data unless the final build does so. | Data safety form is accepted by Play Console. |
